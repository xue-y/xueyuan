...
class XueYuan extends Api
{
    ...
    public function getYingyuNav(){
       ...
       try{
           ...
           if(empty($id)){
               $data=Db::table($nav_table)->field('id,name')->select();

           }else{
               $data=Db::table($nav_table)->where('pid',$id)->field('id,name')->select();
           }
           $this->success('',$data,1);
       }catch (Exception $e){
    ...
    public function getYingyuType(){
        try{
            $nav_table='xueyuan_yingyu_nav';
            $data=Db::table($nav_table)->where('pid',0)->field('id,name')->select();
            $this->success('',$data,1);
        }catch (Exception $e){
            $this->error($e->getMessage(),[],0);
        }
    ...
    public function getYingyuArcCard(){
        ...
        try{
            ...
            if(empty($id)){
                ...
                $data=Db::table($arc_table)->where('aid',$id)->field('id,title')->order('id')->select();
            }
            $this->success('',$data);
        }catch (Exception $e){
    ...
    public function getYingyuCard(){
        ...
        $id=request()->post('cid/d',3); //xueyuan_yingyu_arc 取得指定版块
        if(empty($id)){
            $this->error('文章版块id参数不得为空');
        }
        try{
            $card['contentData']=Db::table($arc_table)->where('id',$id)->field('title,desc,zhishidian,fuxierror,fuxistrong')->order('id')->find();
            // 取得题型
            $card['tixing']=Db::table($tixing_table)->where('cid',$id)->field('id,type,act')->select();
            $i=0;
            foreach ($card['tixing'] as $k=>$v){
                $card['tixing'][$i]['timu']=Db::table($timu_table)->where('tid',$v['id'])->field('tid',true)->select();
                $i++;
            }
            $this->success('',$card);
        }catch (Exception $e){
